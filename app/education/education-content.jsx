    ...
    <section className="flex flex-col items-center text-center w-full max-w-6xl mx-auto px-4 pt-4 pb-16 gap-10">
      ...
      <div className="w-full">
        <ul className="timeline timeline-vertical timeline-compact md:timeline-normal">
          {education.map((e, i) => (
            <li key={i}>
              <div className="timeline-start timeline-box bg-base-100 border border-base-300 p-5 text-left rounded-xl shadow-sm">
                <h3 className="font-bold text-lg">{e.school}</h3>
                <p className="text-sm text-primary font-medium">
                  {e.program} ({e.period})
                </p>
                {e.details && (
                  <p className="mt-2 text-sm text-base-content/70">{e.details}</p>
                )}
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {e.badges.map((tag, ti) => (
                    <span
                      key={ti}
                      className="badge badge-ghost badge-sm text-base-content/70"
                    >
                      {tag}
                    </span>
                  ))}
      ...
      <div className="w-full mt-10">
        <h2 className="text-2xl font-bold mb-8 text-left border-b border-base-300 pb-2">
          Cursos Realizados
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {courses.map((c, i) => (
            <article
              key={i}
              className="card bg-base-100 border border-base-300 rounded-2xl shadow-sm hover:shadow-md transition-shadow"
            >
              <button
                type="button"
                className="w-full h-44 bg-white flex items-center justify-center p-4 rounded-t-2xl cursor-pointer hover:bg-base-200 transition-colors"
                onClick={() => setLightbox({ open: true, src: c.image, alt: c.title })}
              ...
              <div className="card-body p-5 text-left gap-1">
                ...
                <p className="text-sm text-base-content/70 line-clamp-2 mt-1">{c.description}</p>
                <div className="flex flex-wrap gap-1 mt-2">
                  {c.tags.map((t, ti) => (
                    <span
                      key={ti}
                      className="badge badge-outline border-base-300 badge-xs text-base-content/60"
                    >
                      {t}
                    </span>
                  ))}
